...
import static com.github.shangtanlin.config.mq.CartMQConfig.MAX_RETRY_COUNT;
...
public class CartConsumer {
    ...
    public void handleCartWriteBack(CartWriteBackMessage cartWriteBackMessage,
        ...
        try {
            ...
            log.info(">>>> 消息处理成功并已签收，消息ID: {}", msgId);

        } catch (Exception e) {
            log.error(">>>> [业务报错] 消息ID: {}, 重试次数: {}, 错误: {}, 时间: {}",
                    msgId, retryCount, e.getMessage(), LocalDateTime.now().format(FORMATTER));
            ...
            if (retryCount >= MAX_RETRY_COUNT) {
                ...
                rabbitMQConsumeUtils.handleFinalFailure(cartWriteBackMessage, 0,
                CartMQConfig.CART_EXCHANGE,
                CartMQConfig.CART_ROUTING_KEY,
                MAX_RETRY_COUNT,
                e.getMessage(),
                msgId);
